...
async function performSwap(ctx, holdings) {
    ...
    // ── COLLECT ALL → CC branch (opsi 7: loop sampai USDCx & CETH = 0) ──
    if (SWAP_PATTERN === 'D') {
        await runPatternD(ctx, {
            pair_a, pair_b, pair_c,
            refreshBalances, doLeg,
            getCcBalance: () => ccBalance,
            getUsdcxBalance: () => usdcxBalance,
    ...
    // ── USDCx ↔ CETH ANTI RATE-LIMIT branch (opsi 8: loop bolak-balik) ──
    if (SWAP_PATTERN === 'UCE8') {
        await runUsdcxCethSwap(ctx, {
            rounds, delay_min_seconds, delay_max_seconds,
            pair_b, pair_c,
            refreshBalances, doLeg,
            getUsdcxBalance: () => usdcxBalance,
...
// ── Opsi 8: USDCx ↔ CETH Anti Rate-Limit (LOOP) ─────────────────────────
//
// Terus-menerus menukar max amount USDCx→CETH→USDCx→CETH... tanpa henti.
// Anti rate-limit: jika 429 → switch ke arah sebaliknya atau tunggu eskalasi.
// Hanya berhenti jika saldo kedua token = 0 (habis kena fee) atau rounds habis.

async function runUsdcxCethSwap(ctx, params) {
    const { session, walletApi, swapApi, log, index } = ctx;
    const {
        rounds, delay_min_seconds, delay_max_seconds,
        pair_b, pair_c,
        refreshBalances, doLeg,
        getUsdcxBalance, getCethBalance,
        rewardThreshold,
    } = params;

    if (!pair_c) {
        log('⚠️ [8] pair_c (cETH) belum setup. Lakukan 1x swap cETH manual via web dulu.');
        dashboard.update(index, { status: 'error: no cETH' });
        return;
    }

    log('🔄 Opsi 8: USDCx ↔ CETH anti rate-limit dimulai');
    log(`ℹ️ Swap max amount bolak-balik USDCx↔CETH tanpa henti (${rounds} rounds)`);
    dashboard.update(index, { status: 'U↔E loop' });

    await refreshBalances();

    const USDCX_MIN = 0.0001;
    const CETH_MIN  = 0.000001;
    const SWITCH_COOLDOWN_MS = 1000; // 1 detik antar switch pair (anti rate-limit)
    ...
    // Dua pair: USDCx→CETH dan CETH→USDCx
    const PAIRS = [
        { from: pair_b, to: pair_c, key: 'U>E', label: 'U→E', getBalance: getUsdcxBalance, minBal: USDCX_MIN },
        { from: pair_c, to: pair_b, key: 'E>U', label: 'E→U', getBalance: getCethBalance,  minBal: CETH_MIN  },
    ];

    let totalSwaps = 0;
    // Mulai dari pair yang punya saldo lebih besar
    let dirIdx = getUsdcxBalance() >= USDCX_MIN ? 0 : 1;
    ...
    for (let iter = 1; iter <= rounds; iter++) {
        ...
        const uBal = getUsdcxBalance();
        const eBal = getCethBalance();

        // Berhenti jika kedua token habis
        if (uBal < USDCX_MIN && eBal < CETH_MIN) {
            log(`⚠️ [8] Kedua token habis. Selesai! Total: ${totalSwaps} swap`);
            dashboard.update(index, { status: 'done', totalSwaps });
            return;
        }

        log(`🔄 [8] Round ${iter}/${rounds} | USDCx: ${uBal.toFixed(6)} | CETH: ${eBal.toFixed(8)}`);

        // ── Pilih pair kandidat yang punya saldo ──
        const withBalance = PAIRS.filter(p => p.getBalance() >= p.minBal);
        const ready = withBalance.filter(p => !recentRL.has(p.key));

        if (!ready.length) {
            // Semua pair sudah dicoba di burst ini
            if (withBalance.length > 0) {
                if (rlSeenInBurst) {
                    const sec = normalRateLimitSec();
                    const lbl = sec >= 60 ? `${Math.ceil(sec / 60)}m` : `${sec}s`;
                    log(`🔒 [8] Semua pair rate-limited → wait ${lbl}`);
                    dashboard.update(index, { status: `8: wait-rl ${lbl}` });
                    await sleep(sec * 1000);
                } else {
                    log(`⚠️ [8] Semua pair gagal (non-429) → wait 30s`);
                    dashboard.update(index, { status: '8: wait 30s' });
                    await sleep(30 * 1000);
                }
                recentRL.clear();
                rlSeenInBurst = false;
            } else {
                log(`ℹ️ [8] Tidak ada saldo di kedua token, tunggu 15s...`);
                await sleep(15 * 1000);
            }
            continue;
        }

        // Pilih pair: utamakan arah saat ini (dirIdx), jika tidak ready switch
        let chosen = ready.find(p => p.key === PAIRS[dirIdx].key) || ready[0];
        const amount = chosen.getBalance();

        log(`💱 [8] ${chosen.label} | amount: ${amount.toFixed(8)}`);
        dashboard.update(index, { status: `8: ${chosen.label}` });

        let swapOk = false;
        try {
            const r = await doLeg(chosen.from, chosen.to, amount, `8#${iter} ${chosen.label}`, {
                pollTimeoutMinutes: 10,
                throwOn429: true,
                fastSetupFail: true,
            });
            ...
            if (is429) {
                log(`⚠️ [8] 429 rate-limit pada ${chosen.label} → switch pair`);
                recentRL.add(chosen.key);
                rlSeenInBurst = true;
                await sleep(SWITCH_COOLDOWN_MS);
                continue;
            }
            log(`⚠️ [8] Error ${chosen.label}: ${err?.message || err}`);
            recentRL.add(chosen.key);
            await sleep(SWITCH_COOLDOWN_MS);
            continue;
        }
        ...
        if (swapOk) {
            ...
            log(`✅ [8] ${chosen.label} selesai (+${totalSwaps} swap)`);

            // Flip arah untuk round berikutnya
            dirIdx = dirIdx === 0 ? 1 : 0;

            await refreshBalances();
            const d = getRandomDelay(delay_min_seconds, delay_max_seconds);
            log(`⏳ [8] Jeda ${d}s sebelum round berikutnya...`);
            dashboard.update(index, { status: `8: jeda ${d}s` });
            await sleep(d * 1000);
        } else {
            log(`⚠️ [8] ${chosen.label} gagal, coba pair lain`);
            recentRL.add(chosen.key);
            await sleep(SWITCH_COOLDOWN_MS);
        }
    }

    log(`🏁 [8] Selesai! Total ${totalSwaps} swap`);
    dashboard.update(index, { totalSwaps, status: 'done' });
}

// ── Pattern D: Collect All → CC (LOOP) ───────────────────────────────────
//
// Loop terus-menerus sampai saldo USDCx DAN CETH = 0 di setiap akun.
// Urutan per iterasi: CETH → CC dulu, lalu USDCx → CC.
// Jika swap gagal → retry dengan jeda eskalasi (tidak berhenti).
// Jika kedua saldo = 0 → akun selesai (done).

async function runPatternD(ctx, params) {
    const { session, walletApi, swapApi, log, index } = ctx;
    const {
        pair_a, pair_b, pair_c,
        refreshBalances, doLeg,
    ...
    log('🧹 Pattern D: Collect All → CC (loop) dimulai');
    log('ℹ️ Bot akan terus berjalan sampai USDCx & CETH = 0');
    dashboard.update(index, { status: 'collect→CC' });
    ...
    while (true) {
        ...
        // ── Kondisi selesai: kedua token sudah 0 ──
        if (cethBal <= 0.000001 && usdcxBal < 0.0001) {
            log(`✅ [D] Semua token sudah terkumpul ke CC!`);
            log(`🏁 [D] Selesai! Total ${totalSwaps} swap. CC akhir: ${ccBal.toFixed(4)}`);
            dashboard.update(index, { totalSwaps, status: 'done' });
            return;
        }

        log(`🔄 [D] Iter #${iter} | CC: ${ccBal.toFixed(4)} | USDCx: ${usdcxBal.toFixed(6)} | CETH: ${cethBal.toFixed(8)}`);

        let swappedThisIter = false;

        // ── Step 1: CETH → CC ──
        if (pair_c && cethBal > 0.000001) {
            log(`💱 [D] CETH (${cethBal.toFixed(8)}) → CC`);
            dashboard.update(index, { status: 'D: E→CC' });
            const r = await doLeg(pair_c, pair_a, cethBal, `D#${iter} CETH→CC`, { pollTimeoutMinutes: 10 });
            if (r) {
                totalSwaps++;
                swappedThisIter = true;
                consecutiveFails = 0;
                dashboard.update(index, {
                    totalSwaps,
                    lastSwapDir: 'E→CC',
                ...
                log(`✅ [D] CETH → CC selesai (+${totalSwaps} swap)`);
                await refreshBalances();
            } else {
                consecutiveFails++;
                log(`⚠️ [D] CETH → CC gagal (#${consecutiveFails}), lanjut cek USDCx`);
            }
        }

        // ── Step 2: USDCx → CC ──
        const usdcxBalNow = getUsdcxBalance();
        if (usdcxBalNow >= 0.0001) {
            log(`💱 [D] USDCx (${usdcxBalNow.toFixed(6)}) → CC`);
            dashboard.update(index, { status: 'D: U→CC' });
            const r = await doLeg(pair_b, pair_a, usdcxBalNow, `D#${iter} USDCx→CC`, { pollTimeoutMinutes: 10 });
            if (r) {
                totalSwaps++;
                swappedThisIter = true;
                consecutiveFails = 0;
                ...
                log(`✅ [D] USDCx → CC selesai (+${totalSwaps} swap)`);
                await refreshBalances();
            } else {
                consecutiveFails++;
                log(`⚠️ [D] USDCx → CC gagal (#${consecutiveFails})`);
            }
        }

        // ── Jika tidak ada yang bisa diswap iter ini ──
        if (!swappedThisIter) {
            const waitSec = Math.min(15 * (consecutiveFails || 1), MAX_FAIL_WAIT);
            log(`⏳ [D] Tidak ada swap berhasil, tunggu ${waitSec}s lalu cek ulang...`);
            dashboard.update(index, { status: `D: wait ${waitSec}s` });
            await sleep(waitSec * 1000);
        } else {
            const d = getRandomDelay(
                config.swap.delay_min_seconds,
                config.swap.delay_max_seconds
            );
            log(`⏳ [D] Cek ulang saldo dalam ${d}s...`);
            dashboard.update(index, { status: `D: cek ulang` });
            await sleep(d);
        }
    }
}
...
// ── Scan Semua Wallet (Balance + Pending Reward) ─────────────────────────
//
// Login ringan per akun (tanpa swap), baca CC/USDCx/CETH + pending reward
// dari leaderboard, update dashboard, tampilkan summary tabel sebelum bot mulai.

async function scanAllWallets(accounts) {
    console.log(chalk.bold.cyan(`\n  🔍 Scanning ${accounts.length} wallet(s)...\n`));

    const results = await Promise.allSettled(
        accounts.map(async (acc, i) => {
            const log = (msg) => dashboard.log(i, msg);
            dashboard.update(i, { status: 'scanning...' });

            try {
                // ── Login ──
                const session = new Session(acc);
                const walletApi = new WalletAPI(acc.proxy || '');
                const swapApi   = new SwapAPI(acc.proxy || '');

                await session.login(walletApi, swapApi, log);

                // ── Balance ──
                const { holdings = {} } = await session.withRetry(
                    () => walletApi.getBalance(session.walletToken),
                    'wallet', walletApi, swapApi, log
                );

                let cc = 0, usdcx = 0, ceth = 0;
                for (const [tok, info] of Object.entries(holdings)) {
                    if (['Amulet', 'CC (Amulet)', 'CC'].includes(tok)) cc = info.balance || 0;
                    if (['USDCx', 'USDCX'].includes(tok)) usdcx = info.balance || 0;
                    if (['cETH', 'CETH', 'Ceth'].includes(tok)) ceth = info.balance || 0;
                }

                // ── Pending Reward (leaderboard) ──
                let pendingReward = 0, rank = 0, monthTxns = 0;
                try {
                    const lb  = await swapApi.getLeaderboard(session.partyId);
                    const me  = lb.requestedAddress || null;
                    if (me) {
                ...
                dashboard.update(i, {
                    cc, usdcx, ceth,
                    pendingReward, rank, monthTxns,
                    totalReward: pendingReward,
                    initialTxns: monthTxns,
                    initialReward: pendingReward,
                    lastKnownReward: pendingReward,
                    status: 'ready',
                });

                log(`✅ CC:${cc.toFixed(2)} U:${usdcx.toFixed(4)} E:${ceth.toFixed(6)} | Pending:${pendingReward.toFixed(4)} CC`);
                return { i, name: acc.name, cc, usdcx, ceth, pendingReward, rank, ok: true };

            } catch (err) {
                dashboard.update(i, { status: 'scan-err' });
                log(`⚠️ Scan gagal: ${formatError(err)}`);
                return { i, name: acc.name, cc: 0, usdcx: 0, ceth: 0, pendingReward: 0, rank: 0, ok: false };
            }
        })
    );

    // ── Tampilkan tabel summary ──
    const rows = results.map(r => r.status === 'fulfilled' ? r.value : { ok: false, name: '?', cc: 0, usdcx: 0, ceth: 0, pendingReward: 0, rank: 0 });

    const col = (s, w) => String(s).padStart(w);
    const header = `  ${'#'.padEnd(4)} ${'Wallet'.padEnd(12)} ${'CC'.padStart(10)} ${'USDCx'.padStart(10)} ${'CETH'.padStart(12)} ${'Pending Rw'.padStart(12)} ${'Rank'.padStart(6)}`;
    console.log(chalk.bold.cyan(header));
    console.log(chalk.gray(`  ${'─'.repeat(header.length - 2)}`));

    let totCC = 0, totU = 0, totE = 0, totRw = 0;
    rows.forEach((r, idx) => {
        totCC += r.cc; totU += r.usdcx; totE += r.ceth; totRw += r.pendingReward;
        const statusMark = r.ok ? '' : chalk.red(' ⚠');
        const rw = r.pendingReward > 0 ? chalk.yellow(r.pendingReward.toFixed(4)) : chalk.gray('0.0000');
        const rnk = r.rank > 0 ? chalk.cyan(`#${r.rank}`) : chalk.gray('-');
        console.log(
            `  ${col(idx + 1, 3)}. ${r.name?.slice(0, 12).padEnd(12)} ` +
            `${col(r.cc.toFixed(2), 10)} ` +
            `${col(r.usdcx.toFixed(4), 10)} ` +
            `${col(r.ceth.toFixed(6), 12)} ` +
            `  ${rw.padStart(12)} ` +
            `  ${rnk}${statusMark}`
        );
    });
    ...
    console.log(
        chalk.bold(
            `  ${'TOT'.padEnd(17)} ` +
            `${col(totCC.toFixed(2), 10)} ` +
            `${col(totU.toFixed(4), 10)} ` +
            `${col(totE.toFixed(6), 12)} ` +
            `  ${chalk.yellow(totRw.toFixed(4)).padStart(12)}`
        )
    );
    console.log('');
}
...
//   'D'    = g) Collect All : pindahkan semua USDCx & cETH → CC (loop)
//   'UCE8' = h) UCE Anti-RL : USDCx↔CETH bolak-balik max amount, anti rate-limit
let SWAP_PATTERN = 'A';

// Tanya user pilih pola swap (0-8) via readline
function askPattern() {
    return new Promise((resolve) => {
        const rl = readline.createInterface({ input: process.stdin, output: process.stdout });
        const ask = () => {
            rl.question(chalk.cyan('  Pilih pola swap [0, 1, 2, 3, 4, 5, 6, 7, 8]: '), (answer) => {
                const a = String(answer || '').trim();
                const map = { '0': 'BULKBACK', '1': 'A', '2': 'CCU', '3': 'UCE', '4': 'CCE', '5': 'SMART', '6': 'AR', '7': 'D', '8': 'UCE8' };
                if (map[a]) {
                    rl.close();
                    resolve(map[a]);
                } else {
                    console.log(chalk.yellow('  ↳ Masukkan 0, 1, 2, 3, 4, 5, 6, 7, atau 8.'));
                    ask();
                }
            });
        };
...
async function main() {
    ...
    console.log(chalk.green(`  7) Collect All → CC : pindahkan semua USDCx & cETH ke CC (loop sampai habis)`));
    console.log(chalk.cyan(`  8) UCE Anti-RL : USDCx ↔ cETH bolak-balik max amount, anti rate-limit\n`));
    ...
    const patternLabel = {
        ...
        'D': 'Collect All → CC (loop sampai USDCx & cETH = 0)',
        'UCE8': 'UCE Anti-RL (USDCx↔cETH max amount, anti rate-limit, looping)',
    }[SWAP_PATTERN];
    ...
    // ── Scan semua wallet: balance + pending reward sebelum swap dimulai ──
    await scanAllWallets(accounts);
